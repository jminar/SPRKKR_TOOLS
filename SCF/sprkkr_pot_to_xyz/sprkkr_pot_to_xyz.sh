#!/bin/bash
# Convert SPRKKR *.pot file (to be passed as first argument) into a XYZ XCrysDen or JMol -compatible *.xyz file.
#
...
#
# No support yet for substitutional disorder i.e. CPA: the script assumes NT = NQ
#
# A.Marmodoro, Praha, FZU, 2020.
...
' $1 | head -n -1 | wc -l)
echo $nsubs 
echo "From sprkkr_pot_to_xyz.sh *.pot -> *.xsf exporter (SMT scaled by 200):"
...
# grep -A$(($nsubs+1)) ZT $1 \         # Grep the atomic number of each sublattice. For disordered ones, only the first one is parsed.
awk -v nsubs=$nsubs '
